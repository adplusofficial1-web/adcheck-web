    ...
    return (
          <main>
                ...
                <div className="max-w-5xl mx-auto px-6 py-14">
                        <p className="text-xs text-tertiary mb-3">หน้าแรก &nbsp;/&nbsp; บทความ</p>p>
                        <h1 className="text-3xl font-medium mb-3">บทความและอัปเดตกฎหมาย</h1>h1>
                  ...
                  {/* Featured article */}
                        <Link
                                  ...
                                  <h2 className="text-2xl font-medium mb-3">{featured.title}</h2>h2>
                                  <p className="text-secondary mb-4">{featured.excerpt}</p>p>
                                  <div className="flex flex-wrap items-center gap-x-4 gap-y-1">
                                              <SourceBadge article={featured} />
                                              <span className="text-xs text-tertiary">{formatThaiDate(featured.publishedAt)}</span>span>
                                  </div>div>
                        </Link>Link>
                  ...
                  {/* Rest of the list */}
                        ...
                        <Link
                                      ...
                                      <h3 className="text-lg font-medium mb-2">{article.title}</h3>h3>
                                      <p className="text-sm text-secondary mb-4">{article.excerpt}</p>p>
                                      <div className="flex flex-col gap-1">
                                                      <SourceBadge article={article} />
                                                      <span className="text-xs text-tertiary">{formatThaiDate(article.publishedAt)}</span>span>
                                      </div>div>
                        </Link>Link>
                      ))}
                        </div>div>
                ...
                <footer className="bg-inverse text-onInverse text-center py-12 mt-8">
                        <h3 className="text-lg font-medium mb-4">อยากให้ AdCheck ช่วยตรวจสอบโฆษณาก่อนเผยแพร่ไหม</h3>h3>
